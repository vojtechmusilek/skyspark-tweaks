class EditorMatchDoEnd {
  ...
  _matchKeywords(parent) {
    ...
    $(parent).find("span.cm-keyword").each((index, node) => {
      if ($(node).text() == "do") {
        ...
        i--;
        matching.push({ do: doKeywords[i], end: node });
        if (this._doOverlap(cursorPos, node.getBoundingClientRect())) overlap = node;
      }
    })
  ...
  _doOverlap(domRect1, domRect2, margin = 2) {
    var l1 = { x: domRect1.x - margin, y: domRect1.y + domRect1.height };
    var r1 = { x: domRect1.x + domRect1.width + margin, y: domRect1.y + domRect1.height - domRect1.height };
    var l2 = { x: domRect2.x - margin, y: domRect2.y + domRect2.height };
    var r2 = { x: domRect2.x + domRect2.width + margin, y: domRect2.y + domRect2.height - domRect2.height };
